...
"""
    plan = await call_llm(planner_prompt, json_mode=True)
    if plan.get("error"):
        return await process_query(q.question, q.session_id, "chat", False)
    ...
    cache_key = hashlib.sha256(f"{q.question}:{intent}:{mem_hash}".encode()).hexdigest()
    if redis_client:
        cached = await redis_client.get(cache_key)
        if cached:
            return json.loads(cached)

    result = await process_query(q.question, q.session_id, intent, is_new_topic)
...
async def get_dashboard_data():
    ...
    try:
        cur.execute("""
            ...
            FROM sales
            WHERE date_time >= date('now', '-5 months')
            GROUP BY month
            ORDER BY month
        """)
        sales_rows = cur.fetchall()
        sales_labels = [r["month"] for r in sales_rows]
        sales_data = [float(r["revenue"]) for r in sales_rows]
    ...
    try:
        cur.execute("""
            SELECT strftime('%Y-%m', s.date_time) as month,
                   COALESCE(SUM((si.selling_price - si.cost_price) * si.quantity), 0) as profit
            FROM sale_items si
            JOIN sales s ON si.sale_id = s.id
            WHERE s.date_time >= date('now', '-5 months')
            GROUP BY month
            ORDER BY month
        """)
        profit_rows = cur.fetchall()
        profit_labels = [r["month"] for r in profit_rows]
        profit_data = [float(r["profit"]) for r in profit_rows]
    except Exception as e:
        profit_labels = []
        profit_data = []
    ...
    except: total_customers = 0
    try:
        cur.execute("SELECT COUNT(*) FROM sales WHERE date_time >= date('now', '-30 days')")
        orders_last_30d = cur.fetchone()[0] or 0
    except: orders_last_30d = 0
    try:
        cur.execute("SELECT COALESCE(SUM(total_amount), 0) FROM sales WHERE date_time >= date('now', '-30 days')")
        revenue_last_30d = cur.fetchone()[0] or 0
    except: revenue_last_30d = 0
    try:
        cur.execute("SELECT COALESCE(SUM((selling_price - cost_price) * quantity), 0) FROM sale_items")
        total_profit = cur.fetchone()[0] or 0
    except: total_profit = 0
    try:
        cur.execute("SELECT COUNT(*) FROM stock WHERE quantity < 20")
        low_stock_items = cur.fetchone()[0] or 0
    except: low_stock_items = 0
    
    conn.close()
    
    return {
        "salesTrend": {"labels": sales_labels, "data": sales_data},
        "profitTrend": {"labels": profit_labels, "data": profit_data},
